import { Alert,View, Text, useWindowDimensions, SafeAreaView, StatusBar, StyleSheet, Image, ScrollView, TextInput, TouchableOpacity } from 'react-native'
import React, {useState, useEffect} from 'react'
import {heightPercentageToDP as hp, widthPercentageToDP as wp} from 'react-native-responsive-screen';
import CustomBtn from '../components/CustomBtn';
import OTPTextInput from 'react-native-otp-textinput';
import { verifyOtp, signup, sendOtp } from '../middlewares/api';
import CustomLoading from '../components/CustomLoading';

export default function OTPScreen({ route, navigation }) {
  const styles = useStyle();
  const [timer, setTimer] = useState(60);
  const { email, password, name, number } = route.params;
  ...
  return (
    ...
    <Text style={styles.navText}>OTP Verification</Text>
    </View>
    </View>
